...
def test_unannotated_single_value_return_type_func():
    ...
    assert_parameters(
        output_space.dimensions[0], d.Float, fs.ID_ROOT_RETURN, None, None, None, None, nullable=False
    )
    ...
    assert_parameters(
        output_space2.dimensions[0], d.Float, fs.ID_ROOT_RETURN, None, None, None, None, nullable=True
    )
    ...
    assert_parameters(
        output_space3.dimensions[0], d.Int, fs.ID_ROOT_RETURN, None, None, None, None, nullable=True
    )
...
def test_annotated_single_value_return_type_func():
    ...
    assert len(output_space.dimensions) == 1
    assert_parameters(
        output_space.dimensions[0], d.Float, fs.ID_ROOT_RETURN, None, None, None, None, nullable=False, label="Zero"
    )

    def f2() -> Annotated[Optional[float], raxpy.Float(label="ZeroOrNone",tags=[raxpy.tags.MAXIMIZE])]:
        return 0.0
    output_space2 = fs.extract_output_space(f2)
    assert output_space2 is not None
    assert output_space2.dimensions is not None
    assert len(output_space2.dimensions) == 1
    assert_parameters(
        output_space2.dimensions[0], d.Float, fs.ID_ROOT_RETURN, None, None, None, None, nullable=True, tags=[raxpy.tags.MAXIMIZE], label="ZeroOrNone"
    )

    def f3() -> Annotated[Union[int,None], raxpy.Integer(label="Int",lb=0,ub=4)]:
        return 0
    output_space3 = fs.extract_output_space(f3)
    assert output_space3 is not None
    assert output_space3.dimensions is not None
    assert len(output_space3.dimensions) == 1
    assert_parameters(
        output_space3.dimensions[0], d.Int, fs.ID_ROOT_RETURN, None, 0, 4, None, nullable=True, label="Int"
    )